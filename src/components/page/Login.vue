        ...
        <div class="ms-login">
            ...
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm">
                ...
                <el-form-item prop="password">
                    <el-input type="password" placeholder="密码" ref="mark" v-model="ruleForm.password"
...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (this.judgeLogin()) {
                            ...
                            localStorage.setItem('remember_username', this.ruleForm.username);//记住账号
            ...
            requestLogin() {
                ...
                loginApi(loginParams).then(data => {
                    ...
                    // this.$store.commit('changeRole', data.data.rightarr)
                    // this.$store.state.roleArr = data.data.rightarr;
                    sessionStorage.setItem('userMenu', data.data.rightarr);//本来是要用vuex存储为全局变量，但是刷新后store中的state不知为何会莫名消失
                    if (!data.success) {
                        this.$message({
                            message: data.data,
                            type: 'error'
        ...
        mounted() {
            console.log(this.$refs.mark.$el.querySelector('input'))
            this.$refs.mark.$el.querySelector('input').focus();
            this.ruleForm.username = localStorage.getItem("remember_username")
