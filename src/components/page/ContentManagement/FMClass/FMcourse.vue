	...
	<div class="table">
		<div class="container">
			...
			<el-table :data="list" :max-height="680" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange" :header-cell-style="{color:'#000',font_weight:800,background:'#f6f6f6'}">
				...
				<el-table-column prop="id" label="套课ID" min-width="70" fixed></el-table-column>
				<el-table-column prop="name" label="套课名称" min-width="150" :show-overflow-tooltip="true"></el-table-column>
				...
				<el-table-column prop="cover_img" label="套课纵图" min-width="80">
				...
				<el-table-column prop="status" label="上线状态" width="70" :formatter="formatStatus"></el-table-column>
				<el-table-column prop="is_limit_free" label="限时免费" width="70" :formatter="formatIs"></el-table-column>
				<el-table-column prop="ori_price" label="原始价格" width="70"></el-table-column>
				<el-table-column prop="sale_price" label="销售价格" width="70"></el-table-column>
						...
						<el-form :model="addForm" ref="addForm" label-position="left" label-width="120px">
							...
							<el-form-item label="套课标签:" prop="lables">
								<el-checkbox-group v-model="lablesdata" size="mini">
									<el-checkbox v-for="(lis) in dlist" :key="lis.lable_name" :label='lis.lable_name' border @change="clickindex(lis)"></el-checkbox>
								</el-checkbox-group>
							</el-form-item>
							<el-form-item label="订单支付图:" prop="avatar">
								<el-input v-model="addForm.avatar"></el-input>
								<el-button @click="selectMedia" type="primary" class="selectFileBtn">选取文件</el-button>
							</el-form-item>
							<el-form-item label="近期更新图:" prop="banner">
								<el-input v-model="addForm.banner"></el-input>
								<el-button @click="selectMedia2" type="primary" class="selectFileBtn">选取文件</el-button>
							</el-form-item>
							<el-form-item label="热门精品:" prop="hotcourse_img">
								<el-input v-model="addForm.hotcourse_img"></el-input>
							...
							<el-form-item label="套课分享链接图:" prop="fangtu_img">
							...
							<el-form-item label="内容:" prop="content">
								<template slot-scope="scope">
									<span @click="isEditShow(scope.$index, scope.row)" style="cursor: pointer" v-model="addForm.content" ref="content">
                  点击编辑内容</span>
								</template>
							</el-form-item>
							...
							<el-form-item label="上课热门课程:" prop="home">
								<el-select v-model="addForm.home">
									<el-option v-for="item in HomeOptions" :key="item.value" :label="item.label" :value="item.value">
									</el-option>
								</el-select>
							</el-form-item>
							<el-form-item label="限时免费:" prop="is_limit_free">
								<el-select v-model="addForm.is_limit_free">
			...
			<media-select v-if="isSelect4" @close="closeMedia" @overSelectMedia="overSelectMedia4" @overSelectFile="overSelectFile4"></media-select>
			<media-select v-if="isSelect5" @close="closeMedia" @overSelectMedia="overSelectMedia5" @overSelectFile="overSelectFile5"></media-select>
			<media-select v-if="isSelect6" @close="closeMedia" @overSelectMedia="overSelectMedia6" @overSelectFile="overSelectFile6"></media-select>
			<!--进度条弹框-->
...
<script>
	...
	import { GetLables, getCourses, getCoursesOne, deleteCoursesOne, postFm, putFm, postCoursesList, getPicURL, getFM, getFmOne, deleteFmOne, searchAll } from '../../../../api/api';
	...
	export default {
		...
		methods: {
			...
			overSelectMedia3(str) {
				this.addForm.hotcourse_img = str
			},
			overSelectFile3(str) {
				this.addForm.hotcourse_img = str
			},
			...
			async getData() {
				await getFM(this.cur_page, this.size).then(res => {
					console.log("我是打印的data" + res.data.data);
			...
			handleEdit(index, row) {
				...
				this.addForm = {
					...
					hotcourse_img: row.hotcourse_img,
					...
					home: row.home,
					is_limit_free: row.is_limit_free,
			...
			//修改
			myPutCoursesChange() {
				putFm(this.addForm, this.addForm.id).then((res) => {
					if(res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
