	...
	<div class="table">
		<div class="container">
			...
			<el-table :data="list" border max-height="680" style="width: 100%" ref="multipleTable"
				...
				<el-table-column prop="name" label="FM名称" min-width="180" fixed :show-overflow-tooltip="true"></el-table-column>
				<el-table-column prop="lecturer" label="主讲人" min-width="100"></el-table-column>
				<el-table-column prop="audio_time" label="音频时长" min-width="60"></el-table-column>
				<el-table-column prop="viewed_num" label="观看人数" min-width="100"></el-table-column>
				<el-table-column prop="avatar" label="方图" min-width="100">
					<template slot-scope="scope">
						<a :href="scope.row.fangtu_img" target="true"><img :src="scope.row.fangtu_img" height="90%" /></a>
					</template>
				</el-table-column>
				<el-table-column prop="ori_price" label="原始价格" min-width="100"></el-table-column>
				<el-table-column prop="sale_price" label="销售价格" min-width="100"></el-table-column>
				...
				<el-table-column prop="recommend" label="是否推荐" :formatter="formatRecommend"></el-table-column>
				...
				<div class="categories-news-main">
					...
					<div class="categories-news-content">
						<el-form :model="addForm" :rules="addFormRules" ref="addForm" label-position="left" label-width="80px">
							...
							<el-form-item label="试听类型:" prop="type">
...
<script>
	...
	import { getCourses, getCoursesOne, deleteCoursesOne, DeleteFmClass, postFm2, putFm2, postFm2List, getPicURL, getAudioURL, getFmClass, searchClassAll } from '../../../../api/api';
	//import {get} from '../../../../api/index';
	import { formatDate, handleCommonAvatarSuccess, handleCommonAudioSuccess } from '../../../Utility/Utility'
	import { get } from '../../../../api';
	require('../../../../../static/css/myyinru.css');
	...
	export default {
		...
		methods: {
			...
			async getData() {
				// await getAudioavinfo('http://zzaudio.wezhenzhi.com/1544005887730.mp3').then((res) =>{
				//   debugger
				//   console.log(res+"b时间")
				// }).catch((error) => {
				//   console.log("请求错误"+error);
				// })
				await getFmClass(this.cur_page, this.size).then(res => {
					//							 console.log(res.data);
			...
			//修改
			myputFm2Change() {
				putFm2(this.addForm, this.addForm.id).then((res) => {
