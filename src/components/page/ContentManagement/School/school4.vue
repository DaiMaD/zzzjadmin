  ...
  <div class="table">
    <div class="container">
      ...
      <el-table :data="list" border max-height="680" style="width: 100%" ref="multipleTable"
        ...
        <el-table-column prop="name" label="小节名称" min-width="100" :show-overflow-tooltip="true"></el-table-column>
        ...
        <el-table-column prop="avatar" label="略缩图" min-width="100">
          ...
          <div class="categories-news-content">
            <el-form :model="addForm" :rules="addFormRules" ref="addForm" label-position="left" label-width="80px">
              ...
              <!--<el-form-item label="内容:" prop="content">
                <template slot-scope="scope">
                  <span
                    @click="isEditShow(scope.$index, scope.row)"
              ...
              <el-form-item label="视频链接:" prop="video_url">
                ...
                <el-upload class="upload-demo" :show-file-list="false" action="" :on-change="handleVideoSuccess"
                  :auto-upload="false">
              ...
              <el-form-item label="PPT上传" prop="ppts">
                ...
                <el-upload class="upload-demo" :show-file-list="false" action="" :on-change="handlePPTSuccess"
                  :auto-upload="false">
                  <el-button slot="trigger" size="small" type="primary">选取文件</el-button>
                </el-upload>
              </el-form-item>
  ...
  export default {
    data() {
      ...
      return {
        ...
        addFormRules: {
          name: [
            { required: true, message: '小节名称不能为空' },
          ...
          position: [
            { required: true, message: '讲师职位不能为空' },
            { validator: validateName, trigger: 'blur' }
          ],
          course_id: [
            { required: true, message: '所属职位不能为空' },
            { validator: validateName, trigger: 'blur' }
          ],
          ori_price: [
    ...
    methods: {
      ...
      async getData() {
        await getsxyclass(this.cur_page, this.size).then(res => {
      ...
      myDelete(index, row) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          deletesxyclass(row.id).then((res) => {
            if (res.success === true) {
              this.select_cate = "";
              this.getData();
            }
      ...
      //新增
      myPostSectionsAdd() {
        postsxyclass(this.addForm).then((res) => {
          console.log(res)
          if (res.success === true) {
            this.addFormVisible = false;
            this.getData();
      ...
      //修改
      myPutSectionsChange() {
        postsxyclassupdata(this.addForm, this.addForm.id).then((res) => {
          console.log(res.data);
          if (res.success === true) {
            this.addFormVisible = false;
            this.getData();
