  ...
  <div class="table">
    <div class="container">
      ...
      <el-table :data="list" border max-height="680" style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange" :header-cell-style="{color:'#000',font_weight:800,background:'#f6f6f6'}">
        ...
        <el-table-column prop="head_img" label="头图" min-width="180">
        ...
        <el-table-column prop="course_brief" label="课程要点" min-width="00"><template slot-scope="scope">
          ...
          <div class="categories-news-content">
            <el-form  :model="addForm" :rules="addFormRules" ref="addForm" label-position="left" label-width="80px">
              ...
              <el-form-item   label="老师简介:" prop="teacher_brief" >
                <el-input type="textarea" autosize placeholder="请输入内容" v-model.string="addForm.teacher_brief"></el-input>
              </el-form-item>
...
<script>
  ...
  import {getSxyshare,posteradd,posterupdate,getPicURL,DeletePoster} from '../../../../api/api';
  ...
  export default {
    ...
    methods: {
      ...
      handleAvatarSuccess(file, fileList) {
          ...
          var complete = function(res) {
            if (res.key && res.key.match(/\.(jpg|jpeg|png)$/)) {
              ...
              getPicURL(res.key).then((res)=>{
                // console.log(res.data);
                _this.progressVisible = false;
                _this.addForm.head_img = res.data
              })
            }
          };
          var error = function(err) {
       ...
       //fm上传缩略图
      handleAvatarSuccesstimg(file, fileList) {
        // console.log(file.name + file.size);
        this.progressVisible = true;
        if (file.name && file.name.match(/\.(jpg|jpeg|png)$/)) {
          var _this = this;
          var complete = function(res) {
            if (res.key && res.key.match(/\.(jpg|jpeg|png)$/)) {
              ...
              getPicURL(res.key).then((res)=>{
                // console.log(res.data);
                _this.progressVisible = false;
                _this.addForm.second_share_img = res.data
              })
            }
          };
          var error = function(err) {
      ...
      async getData(){
        await getSxyshare(this.cur_page,this.size,this.parentid).then(res=>{
