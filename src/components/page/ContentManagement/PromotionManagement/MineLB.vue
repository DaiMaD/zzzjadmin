<template>
	<div class="table">
		<div class="container">
			...
			<el-table :data="list" border max-height="680" style="width: 100%" ref="multipleTable" :header-cell-style="{color:'#000',font_weight:800,background:'#f6f6f6'}">
				<el-table-column type="selection" width="55" fixed></el-table-column>
				<el-table-column prop="img_url" label="我的图片" width="150">
					<template slot-scope="scope">
						<a :href="scope.row.img_url" target="true"><img :src="scope.row.img_url" width="90%" style="vertical-align: middle;" /></a>
					</template>
				</el-table-column>
			...
			<div class="pagination">
				<el-pagination background @current-change="handleCurrentChange" layout="total, prev, pager, next" :total="this.total">
				</el-pagination>
			</div>
			<!--新增弹框-->
...
<script>
	...
	import { getPosterMytab, putPosterMytab, postPosterMytab, DelPosterMytab, getPicURL } from '../../../../api/api';
	...
	export default {
		...
		methods: {
			...
			//获取数据
			async getData() {
				await getPosterMytab(this.cur_page, this.size).then(res => {
			...
			myPutActivityChange() {
				...
				putPosterMytab(this.addForm.id, addForm3).then((res) => {
					if(res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
			...
			myPostActivityAdd() {
				...
				postPosterMytab(addForm1).then((res) => {
					this.getData();
					if(res.success === true) {
						this.addFormVisible = false;
						this.getData();
			...
			myDelete(index, row) {
				...
				this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					DelPosterMytab(row.id).then((res) => {
						console.log(res)
						if(res.success === true) {
							this.select_cate = '';
							this.getData();
