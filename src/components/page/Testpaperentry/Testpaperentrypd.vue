...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				await getpanduanti(this.cur_page, this.size).then(res => {
					if (!res.data.data ||
						res.data.data === [] ||
			...
			search() {
				if (this.examContent != '') {
					searchpanduanti2(this.examContent, 1, this.size).then(res => {
						console.log(res)
						this.list = res.data.data;
					});
				} else if (this.examContent2 != '') {
					searchpanduanti3(this.examContent2, 1, this.size).then(res => {
						console.log(res)
						this.list = res.data.data;
					});
				}
			...
			selectStatus(value) {
				...
				searchpanduanti(value, 1, this.size).then(res => {
					console.log(res)
					this.list = res.data.data
				})
			},
			...
			myPostCoursesAdd() {
				...
				addpanduanti(this.addForm).then(res => {
					if (res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
			...
			//----------选择题修改--------------
			myPutCoursesChange() {
				putpanduanti(this.addForm, this.addForm.id).then(res => {
					if (res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
