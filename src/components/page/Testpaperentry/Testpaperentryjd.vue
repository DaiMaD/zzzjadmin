...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				await getjiandati(this.cur_page, this.size).then(res => {
					if (!res.data.data ||
						res.data.data === [] ||
			...
			search() {
				if (this.examContent != '') {
					searchjiandati2(this.examContent, 1, this.size).then(res => {
						console.log(res)
						this.list = res.data.data;
					});
				} else if (this.examContent2 != '') {
					searchjiandati3(this.examContent2, 1, this.size).then(res => {
						console.log(res)
						this.list = res.data.data;
					});
				}
			...
			myDelete(index, row) {
				...
				this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
					...
					.then(() => {
						Deletejiandati(row.id).then(res => {
							if (res.success === true) {
								this.select_cate = "";
								this.getData();
							}
			...
			myPostCoursesAdd() {
				...
				addjiandati(this.addForm).then(res => {
					if (res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
			...
			selectStatus(value) {
				...
				searchjiandati(value, 1, this.size).then(res => {
					console.log(res)
					this.list = res.data.data
				})
			},
			//----------简答题修改--------------
			myPutCoursesChange() {
				putjiandati(this.addForm, this.addForm.id).then(res => {
					if (res.success === true) {
						this.addFormVisible = false;
						this.getData();
					} else {
