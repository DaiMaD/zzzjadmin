    ...
    <div class="crumbs">
      ...
      <div class="container">
        <div class="handle-box">
          <el-input size="medium" v-model="examContent" placeholder="根据题干关键字查询" class="handle-select mr10" clearable
            ref='examcontent'></el-input>
          <el-input size="medium" v-model="examContent2" placeholder="课程名称查询" class="handle-select mr10" clearable
            ref='examcontent2'></el-input>
        ...
        <el-table :data="list" border stripe max-height="400" style="width: 100%" ref="multipleTable"
          ...
          <el-table-column prop="answer" label="正确答案" width="70" :show-overflow-tooltip="true"></el-table-column>
              ...
              <el-form :model="addForm" :rules="addFormRules" ref="addForm" label-position="left" label-width="120px">
                <el-form-item label="证书类型:" prop="certificate_type">
                  ...
                  <el-checkbox-group v-model="checkType" @change="checkMoreChange">
                    <el-checkbox v-for="items in getZhengshuData" :label="items.subject" :key="items.value"
                      class="check_type"></el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getData() {
        await getzongheti(this.cur_page, this.size).then(res => {
          if (
            !res.data.data ||
            res.data.data === [] ||
      ...
      search() {
        if (this.examContent != '') {
          searchzongheti2(this.examContent, 1, this.size).then(res => {
            console.log(res)
            this.list = res.data.data;
          });
        } else if (this.examContent2 != '') {
          searchzongheti3(this.examContent2, 1, this.size).then(res => {
            console.log(res)
            this.list = res.data.data;
          });
        }
      ...
      selectStatus(value) {
        ...
        searchzongheti(value, 1, this.size).then(res => {
          console.log(res)
          this.list = res.data.data
        })
      },
      ...
      myDelete(index, row) {
        ...
        this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
          ...
          .then(() => {
            Deletezongheti(row.id).then(res => {
              if (res.success === true) {
                this.select_cate = "";
                this.getData();
              }
      ...
      myPostCoursesAdd() {
        ...
        addzongheti(this.addForm).then(res => {
          if (res.success === true) {
            this.addFormVisible = false;
            this.getData();
          } else {
      ...
      //----------选择题修改--------------
      myPutCoursesChange() {
        putzongheti(this.addForm, this.addForm.id).then(res => {
          if (res.success === true) {
            this.addFormVisible = false;
            this.getData();
          } else {
