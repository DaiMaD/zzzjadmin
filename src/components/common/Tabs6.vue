	...
	export default {
		...
		mounted() {
			bus.$on('navPath', (name, val) => {//处理传过来的值
				console.log(name)
				var titname;
				if (name[name.length - 1] == 'UserList') {
					titname = '用户列表'
				} else if (name[name.length - 1] == 'Collection') {
					titname = '用户收藏'
				} else if (name[name.length - 1] == 'WxUsers') {
					titname = '微信用户'
				}
				if (this.tabList.filter(f => f.name == name[name.length - 1]) == 0) {
					var component = resolve => require([`../page/UserManagement/${name[name.length - 1]}/${name[name.length - 1]}`], resolve)//用户管理
					this.tabList.push({
						title: titname,
