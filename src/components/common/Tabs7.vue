	...
	export default {
		...
		mounted() {
			bus.$on('navPath', (name, val) => {//处理传过来的值
				console.log(name)
				var titname;
				if (name[name.length - 1] == 'AdministratorList') {
					titname = '管理员列表'
				} else if (name[name.length - 1] == 'Help') {
					titname = '帮助信息'
				} else if (name[name.length - 1] == 'AdministratorList2') {
					titname = '管理员列表2'
				} else if (name[name.length - 1] == 'UpdateHistory') {
					titname = '版本更新'
				} else if (name[name.length - 1] == 'Menumanagement') {
					titname = '版本更新'
				}
				if (this.tabList.filter(f => f.name == name[name.length - 1]) == 0) {
					var component = resolve => require([`../page/SystemManagement/${name[name.length - 1]}/${name[name.length - 1]}`], resolve)//合伙人管理
					this.tabList.push({
						title: titname,
